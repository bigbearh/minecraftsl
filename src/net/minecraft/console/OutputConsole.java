...
public class OutputConsole extends WindowAdapter
  implements WindowListener, ActionListener, Runnable
{
  ...
  public OutputConsole()
  {
    ...
    mainPanel.setBorder(new MatteBorder(2, 2, 2, 2, new Color(255, 255, 255, 1)));
    ...
    MouseAdapter mouseListenerTA = new MouseAdapter() {
      public void mouseReleased(MouseEvent e) {
          if (e.isPopupTrigger()) {
              if (textArea.getSelectedText() == null || textArea.getSelectedText() == "") {
    ...
    JScrollPane areaPane = new JScrollPane(this.textArea);
    ...
    versionL.setBorder(new MatteBorder(4, 4, 4, 4, new Color(255, 255, 255, 0)));
  	...
  	dumpfB.addActionListener(new ActionListener()
    {
      public void actionPerformed(ActionEvent e) {
        try {
	        ...
	        HTTPClient.openLink(new URL("file://" + new File(Options.getMCDir(), "DUMPS").getAbsolutePath()).toURI());
        } catch (Exception e1) {
	        e1.printStackTrace();
        }
      }
    ...
    {
      public void actionPerformed(ActionEvent e) {
        StringSelection stringSelection = new StringSelection(textArea.getTXT());
        Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
        clipboard.setContents(stringSelection, null);
      }
  	});
  ...
  public static void dump() {
    ...
    try
    {
        ...
        html = html2[0]+"<font face=\"sans-serif\">MinecraftSL build "+MinecraftSL.build+"<br>Made by the MinecraftSL OpenSource- and User Community<br><br>This is an Output Console Log Dump .<br>If problems were happening create a new issue , see the homepage to see how .<br><br><br><div style=\"height: 1px; font-size: 1px; line-height: 1px; background-color: #000000; overflow:hidden;\"></div><br> "+html2[1];
        String[] lines = html.split("\n");
        int done = 0;
        while (done != lines.length) {
          printwriter.println((new StringBuilder()).append(lines[done]).toString());
          done = done + 1;
        }
        printwriter.close();
	...
	public synchronized void windowClosed(WindowEvent evt)
  {
    this.quit = true;
    try {
      this.reader.join(1L); this.pin.close(); } catch (Exception localException) {
    }try { this.reader2.join(1L); this.pin2.close();
  ...
  public synchronized void run()
  {
    try
    {
      while (Thread.currentThread() == this.reader) {
        try {
          wait(1L); } catch (InterruptedException localInterruptedException) {
      ...
      while (Thread.currentThread() == this.reader2) {
        try {
          wait(1L); } catch (InterruptedException localInterruptedException1) {
        }if (this.pin2.available() != 0)
        {
          String input = readLine(this.pin2);
          log = log + input;
          this.textArea.append(input, Color.red);
        }
        if (this.quit) return; 
      }
    }
