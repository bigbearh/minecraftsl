...
/**
 * Modules ( ModuleUtil ) , Utility for loading external classes ( Mods , Plugins , Addons ) . 
 *	@author Maik
 */
	...
	public static void injectModule(File f) {
		try {
	    	...
	    	while ((entry = zis.getNextEntry()) != null && name.equals("")) {
	    		if (entry.getName().contains("slmod_") && !entry.getName().contains("$") && !entry.isDirectory()) name = entry.getName();
	    	}
	    	String className = name.replaceAll("/", ".").replaceAll("\\\\", ".").replaceAll(".class", "");
			
			final ModuleClassLoader loader = new ModuleClassLoader(new URL[] {f.toURI().toURL()});
