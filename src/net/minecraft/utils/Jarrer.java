...
public class Jarrer {
			...
			try {
				...
				try {
					...
					while (done != filesCount) {
						// if (!add(srcFolder.getPath(), files[done],
						// srcFolder.getPath(), out)) {
	...
	protected static boolean addToJar(String absolutePath, String relativePath,
			String fileName, JarOutputStream out) {
		...
		try {
			...
			if (relativePath.equals("")
				...
				entry = new JarEntry(relativePath.replace("\\", "/")
						+ fileName.replace("\\", "/"));
			} else {
				entry = new JarEntry(relativePath.replace("\\", "/") + "/"
						+ fileName.replace("\\", "/"));
			...
			Logger.getLogger(Jarrer.class.getName())
