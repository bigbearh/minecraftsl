...
public class MinecraftTexpackInstaller extends JFrame
{
  ...
  private void initComponents()
  {
    ...
    this.jLabel4.setText("But I will use .mctexpacks extension,  cuz that makes me happy.");

    this.jLabel5.setText("This Patch makes you backup all texturepacks.");

    this.jLabel6.setText("Ex: If you have 2 texturepacks all of them will be backuped . Otherwise 3 if 3 avaible and so on.");
    ...
    this.jLabel1.setText("You might want to backup your Texturepack-Pack at least once after it's installed.");

    this.jLabel2.setText("Never know when there is something deleting everything.");

    this.jLabel7.setText("Zip format again, but again .mctexpacks extension. It really makes me happy.");

    GroupLayout jPanel2Layout = new GroupLayout(this.jPanel2);
    this.jPanel2.setLayout(jPanel2Layout);
    jPanel2Layout.setHorizontalGroup(jPanel2Layout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(jPanel2Layout.createSequentialGroup().addContainerGap().addGroup(jPanel2Layout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(jPanel2Layout.createSequentialGroup().addGap(6, 6, 6).addGroup(jPanel2Layout.createParallelGroup(GroupLayout.Alignment.LEADING).addComponent(this.jLabel2).addComponent(this.jLabel7).addComponent(this.jLabel1))).addComponent(this.jSeparator1, -1, 551, 32767).addGroup(jPanel2Layout.createSequentialGroup().addComponent(this.uninstall).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED)).addGroup(jPanel2Layout.createSequentialGroup().addPreferredGap(LayoutStyle.ComponentPlacement.RELATED)).addComponent(this.jButton5, -2, 239, -2)).addContainerGap()));

    jPanel2Layout.setVerticalGroup(jPanel2Layout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(jPanel2Layout.createSequentialGroup().addContainerGap().addGroup(jPanel2Layout.createParallelGroup(GroupLayout.Alignment.BASELINE).addComponent(this.uninstall)).addGap(3, 3, 3).addComponent(this.jSeparator1, -2, 10, -2).addGap(18, 18, 18).addGroup(jPanel2Layout.createParallelGroup(GroupLayout.Alignment.BASELINE)).addGap(18, 18, 18).addComponent(this.jButton5).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED).addComponent(this.jLabel1).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED).addComponent(this.jLabel2).addPreferredGap(LayoutStyle.ComponentPlacement.RELATED).addComponent(this.jLabel7).addContainerGap(-1, 32767)));
  ...
  private void uninstallActionPerformed(ActionEvent evt) {
    int result = JOptionPane.showConfirmDialog(this, "Are you sure that you want to uninstall Minecrafts Texturepacks?\nCan't Undo It! (unless you have backup, hehe)", "Are you sure? (Uninstallation)", 0, 2);
  ...
  {
    int result = JOptionPane.showConfirmDialog(this, "Are you sure that you want to install another Texturepack?\nIT MIGHT OVERWRITE YOUR OLD ONE!\nMake sure you have your most recent texture packs backed up before this!", "Are you sure? (Texturepack Install)", 0, 2);
    ...
    try
    {
      ...
      JOptionPane.showMessageDialog(this, "Failed!\nInvalid Zip File!\nOr another unknown error !", "Texturepack Installation", 0);

      return;
    }
    JOptionPane.showMessageDialog(this, "Done!", "Texturepack Installation", -1);
    refreshButtons();
  }
  ...
  private void jButton1ActionPerformed(ActionEvent evt) {
    ...
    if (!new File(Options.getMCDir(), "texturepacks").exists()) {
      JOptionPane.showMessageDialog(this, "Sorry, but you have no texturepacks!", "Texturepack Backup", 0);
      return;
    }
    JFileChooser save = new JFileChooser();
    Calendar now = GregorianCalendar.getInstance();
    save.setFileSelectionMode(0);
    save.setSelectedFile(new File(String.format("MCTexpacks" + world + "_" + "%1$tY-%1$tm-%1$td_%1$tH-%1$tM-%1$tS" + "_Backup." + "mctexpacks", new Object[] { now })));
  ...
  private void jButton2ActionPerformed(ActionEvent evt)
  {
    int world = getWorldSelected();
    if (new File(Options.getMCDir(), "texturepacks").exists()) {
      int result = JOptionPane.showConfirmDialog(this, "Are you sure that you want to overwrite your Texturepacks?\nCan't Undo It! (unless you have backup, hehe)", "Are you sure? (Texturepack Restoration)", 0, 2);
    ...
    try
    {
      ...
      JOptionPane.showMessageDialog(this, "Failed!\nInvalid Zip Contents!\nthe world folder inside must have 'texpacks_backup' as name.", "Texturepack Restoration", 0);

      return;
    }
    JOptionPane.showMessageDialog(this, "Done!", "Texturepack Restoration", -1);
    refreshButtons();
  }
