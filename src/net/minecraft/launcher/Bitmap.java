...
public class Bitmap {
	...
	public static Bitmap tooltipBitmap(int width, int height, int cRadius) {
		int color = Color.black.getRGB();
		Bitmap tooltip = new Bitmap(width+cRadius, height+cRadius);	
		tooltip.fill(0, cRadius, width, height-2*cRadius, color);
		tooltip.fill(cRadius, 0, width-2*cRadius, height, color);
		// draw corner circles
		tooltip.circleFill(cRadius, cRadius, cRadius, color);
	 ...
	 */
	public static BufferedImage convert(Bitmap b) { 
		int w = b.w; 
		int h = b.h;
		
		BufferedImage bi = new BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB);
		bi.setRGB(0,  0, w, h, b.pixels, 0, w);
	 ...
	 */
	public static Bitmap convert(BufferedImage bi) {
	 ...
	 */
	public void replace(int oldc, int newc) {
		for (int i = 0; i < pixels.length; i++) {
			if (pixels[i] == oldc) pixels[i] = newc;
		}
	}

	/**
	 * Twists the image 180 degrees .
	 */
	public void twist180() {
		int[] oldpixels = pixels.clone();
		for (int i = 0; i < pixels.length; i++) {
			pixels[i] = oldpixels[pixels.length - 1 - i];
		}
	}
	...
	public static Bitmap scaleBitmapFactor(Bitmap b, double factor) {
		return scaleBitmap(b, (int)(b.w * factor), (int)(b.h * factor));
	}
